...
| 2-3 | REST API（accounts/journal_entriesをJSON公開、controller/routes/strong params） | 🔵ほぼ完了 |
...
- ⬜ API用401 JSON対応（未ログイン時、今はHTMLログイン画面へリダイレクト）
- request spec計: accounts 5 + journal_entries 3。全体16 examples 0 failures。
- コミット: 98e4d6d(accounts create) / 6d90448(journal_entries create)
