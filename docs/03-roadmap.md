...
| # | 内容 | 鍛える地力 | AI縛り | 状態 |
|---|---|---|---|---|
| **0** | Rails手慣らし（rails new / generate model / db:migrate / console で極小CRUD） | コマンド・リクエストサイクルの体得 | コマンドは教わってよい、コードは手打ち | 進行中 |
| 1 | 複式簿記の台帳コア（Railsのみ・UIなし） | ドメインモデル, 不変条件, DBトランザクション, SQL集計, テスト | **全部手打ち**。AIは環境構築のみ | 未 |
